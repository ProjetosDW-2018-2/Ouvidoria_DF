...
@app.route('/filtroMapaPontos', methods=["GET",'POST'])
def filtroMapaPontos():
    ...
    lista = [comp_select_ano, comp_select_tema, comp_select_sexo, comp_select_tipo_resposta,
             comp_select_situacao_manifestacao]
    cont = 0
    for i in lista:
        if i != None and i != "":
            cont += 1
    if cont >4:
        ...
        resposta = c.fetchall()
        if resposta!= None:
            gerarMapadePonto(resposta, 'mapa_manifestacao_filtro')
            return redirect(url_for('mapa_manifestacao_filtro'))
        flash('Filtro incorreto.', 'erro')
        return render_template('filtroMapaPontos.html', form=form)
    return render_template('filtroMapaPontos.html', form=form)
